...
def load_session():
    """
    Loads the saved session from file if it exists.
    """
    try:
        if os.path.exists(SESSION_FILE):
            with open(SESSION_FILE, 'r') as f:
                session_data = json.load(f)
                auth_config.restore_session(session_data)
                return True
    except Exception as e:
        futil.log(f"Error loading session: {str(e)}")
    return False
...
def save_session():
    ...
    """
    try:
        session_data = auth_config.get_session_data()
        if session_data:
            with open(SESSION_FILE, 'w') as f:
                json.dump(session_data, f)
            futil.log("Session saved successfully")
    except Exception as e:
        futil.log(f"Error saving session: {str(e)}")

def clear_saved_session():
    """
    Clears the saved session file.
    """
    try:
        if os.path.exists(SESSION_FILE):
            os.remove(SESSION_FILE)
            futil.log("Saved session cleared")
    except Exception as e:
        futil.log(f"Error clearing session: {str(e)}")
...
def start():
    ...
    if load_session() and auth_config.validate_session():
        global is_logged_in
        is_logged_in = True
        futil.log("Session restored successfully")
    else:
        clear_saved_session()  # Clear invalid session
...
def command_execute(args: adsk.core.CommandEventArgs):
    ...
    try:
        ...
        if validate_credentials(email, password):
            ...
            if remember_me:
                save_session()
                futil.log("Session saved for future use")
            else:
                clear_saved_session()
